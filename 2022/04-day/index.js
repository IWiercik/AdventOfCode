...
//part one
function gettingOverWorkedFully(array) {
  let overWork = 0;
  array.forEach((team) => {
...
console.log(gettingOverWorkedFully(downloadedData));
...
function gettingOverWorked(array) {
  ...
  array.forEach((team) => {
    ...
    //checking if has item
    for (let j = sortedByAmountOfItems[1][0]; j <= sortedByAmountOfItems[1][1]; j++) {
      if (arrayWithEveryItem.includes(j)) overlapped = true;
    }
    if (overlapped) overWork += 1;
  });
  return overWork;
}
